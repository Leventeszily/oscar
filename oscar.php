...
class Oscar {
    ...
    {
        $query = "SELECT m_ID, title, m_desc, pic, movie_type.Mt_description FROM movie inner join movie_type on movie_type.mt_ID=movie.m_type where movie_type.Mt_name= '".$Mt_name."'";
        $dbvez = new DBVezerlo();
        $this->oscars=$dbvez->executeSelectQuery($query);
        $dbvez->closeDB();
        return $this->oscars;
